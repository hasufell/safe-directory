...
import System.Directory.Internal.OsPath
import qualified System.Directory.OsPath as OSP
...
getPermissions = withOsPath OSP.getPermissions
...
setPermissions path p = withOsPath (flip OSP.setPermissions p) path
...
createDirectory = withOsPath OSP.createDirectory
...
createDirectoryIfMissing :: Bool     -- ^ Create its parents too?
                         ...
                         -> IO ()
createDirectoryIfMissing create_parents = withOsPath (OSP.createDirectoryIfMissing create_parents)
...
removeDirectory = withOsPath OSP.removeDirectory
...
removeDirectoryRecursive = withOsPath OSP.removeDirectoryRecursive
...
removePathForcibly = withOsPath OSP.removePathForcibly
...
removeFile = withOsPath OSP.removeFile
...
renameDirectory opath npath = do
  ...
  OSP.renameDirectory opath' npath'
...
copyFile fromFPath toFPath = do
  ...
  OSP.copyFile fromFPath' toFPath'
...
copyFileWithMetadata src dst = do
  src' <- encodeFS src
  dst' <- encodeFS dst
  OSP.copyFileWithMetadata src' dst'
...
canonicalizePath = \path ->
  decodeFS =<< withOsPath OSP.canonicalizePath path
...
makeAbsolute path = decodeFS =<< withOsPath OSP.makeAbsolute path
...
makeRelativeToCurrentDirectory x = decodeFS =<< withOsPath OSP.makeRelativeToCurrentDirectory x
...
findExecutable binary = maybe (pure Nothing) (fmap Just . decodeFS) =<< withOsPath OSP.findExecutable binary
...
findExecutables binary = do
  res <- withOsPath OSP.findExecutables binary
  forM res decodeFS
...
findExecutablesInDirectories path binary = do
  path' <- forM path encodeFS
  res <- withOsPath (OSP.findExecutablesInDirectories path') binary
  forM res decodeFS
...
findFile path binary = do
  path' <- forM path encodeFS
  res <- withOsPath (OSP.findFile path') binary
  forM res decodeFS
...
findFiles path binary = do
  path' <- forM path encodeFS
  res <- withOsPath (OSP.findFiles path') binary
  forM res decodeFS
...
findFileWith f ds name = do
  ds' <- forM ds encodeFS
  name' <- encodeFS name
  res <- OSP.findFileWith (decodeFS >=> f) ds' name'
  forM res decodeFS
...
findFilesWith f ds name = do
  ds' <- forM ds encodeFS
  name' <- encodeFS name
  res <- OSP.findFilesWith (decodeFS >=> f) ds' name'
  forM res decodeFS
...
-- @since 1.2.4.0
exeExtension :: String
exeExtension = decodeFilepathUnsafe OSP.exeExtension
...
getDirectoryContents fp = withOsPath OSP.getDirectoryContents fp >>= traverse decodeFS
...
listDirectory path = withOsPath OSP.listDirectory path >>= traverse decodeFS
...
getCurrentDirectory = OSP.getCurrentDirectory >>= decodeFS
...
setCurrentDirectory = withOsPath OSP.setCurrentDirectory
...
withCurrentDirectory dir action =
  withOsPath (`OSP.withCurrentDirectory` action) dir

-- | Obtain the size of a file in bytes.
--
-- @since 1.2.7.0
getFileSize :: FilePath -> IO Integer
getFileSize = withOsPath OSP.getFileSize
...
doesPathExist = withOsPath OSP.doesPathExist
...
doesDirectoryExist = withOsPath OSP.doesDirectoryExist
...
doesFileExist = withOsPath OSP.doesFileExist
...
createFileLink target link = do
  ...
  link' <- encodeFS link
  OSP.createFileLink target' link'
...
createDirectoryLink target link = do
  target' <- encodeFS target
  link' <- encodeFS link
  OSP.createDirectoryLink target' link'
...
removeDirectoryLink = withOsPath OSP.removeDirectoryLink
...
pathIsSymbolicLink = withOsPath OSP.pathIsSymbolicLink

{-# DEPRECATED isSymbolicLink "Use 'pathIsSymbolicLink' instead" #-}
isSymbolicLink :: FilePath -> IO Bool
isSymbolicLink = withOsPath OSP.isSymbolicLink
...
getSymbolicLinkTarget fp = withOsPath OSP.getSymbolicLinkTarget fp >>= decodeFS
...
getAccessTime = withOsPath OSP.getAccessTime
...
getModificationTime = withOsPath OSP.getModificationTime
...
setAccessTime path atime = withOsPath (`OSP.setAccessTime` atime) path
...
setModificationTime path mtime = withOsPath (`OSP.setModificationTime` mtime) path
...
getHomeDirectory = OSP.getHomeDirectory >>= decodeFS
...
getXdgDirectory :: XdgDirectory         -- ^ which special directory
                -> FilePath             -- ^ a relative path that is appended
                                        ...
                                        --   path is returned
                -> IO FilePath
getXdgDirectory xdgDir suffix = withOsPath (OSP.getXdgDirectory xdgDir) suffix >>= decodeFS
...
getXdgDirectoryList :: XdgDirectoryList -- ^ which special directory list
                    -> IO [FilePath]
getXdgDirectoryList xdgDirs = OSP.getXdgDirectoryList xdgDirs >>= traverse decodeFS
...
getAppUserDataDirectory :: FilePath     -- ^ a relative path that is appended
                                        --   to the path
                        -> IO FilePath
getAppUserDataDirectory fp = withOsPath OSP.getAppUserDataDirectory fp >>= decodeFS
...
getUserDocumentsDirectory = OSP.getUserDocumentsDirectory >>= decodeFS
...
getTemporaryDirectory = OSP.getTemporaryDirectory >>= decodeFS


withOsPath :: (OsPath -> IO a) -> FilePath -> IO a
withOsPath action fp = do
  os <- encodeFS fp
  action os
